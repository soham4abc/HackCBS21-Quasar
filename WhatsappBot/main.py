...
def mybot():
    ...
    if Sms_Status == "received":
        ...
        if "hi" in incoming_msg:
            msg.body(
                "Hi! 👋 \nI'm *Whatsapp Bot* 🚑 \nA Wapp-Bot 🤖 developed by Team : *Quasar*"
            )
            responded = True
        ...
        if "vaccine" in incoming_msg:
            ...
            msg.body(
                "For nearest Vaccination centre, 💉 \nShare your *Current Location* 📍"
            )
            responded = True
        ...
        if (previouswappmsg == "sos") & (latitude != None):
            ...
            (
                ...
                hospital_address,
                hospital_rating,
            ) = googleapi.placedetails(latitude, longitude, 50000)
            ...
            adf = pandas.read_csv(
                "C:/xampp/htdocs/HackCBS/HackCBS/WhatsappBot/ambulance.csv"
            )
            ambdata = adf.iloc[
                (adf["latitude"] - float(hospital_latitude)).abs().argmin()
            ]
            ...
            (
                ...
                a2pdistance,
                a2pduration,
            ) = googleapi.distance2point(
                Ambulancelatitude, Ambulancelongitude, latitude, longitude
            )
            (
                patient_address,
                hosp_address,
                ...
                latitude, longitude, hospital_latitude, hospital_longitude
            )
            ...
            sd.sendmsglocation(
                Ambulancephoneno, "Location of Patient", latitude, longitude
            )

            # ========================= Searches for Nearest Police Station to Patient using Latitude from police.csv

            df = pandas.read_csv(
                "C:/xampp/htdocs/HackCBS/HackCBS/WhatsappBot/police.csv"
            )
            ...
            nearestdata2 = df.iloc[
                (df["latitude"] - float(hospital_latitude)).abs().argmin()
            ]
